...
The state $d'$ is a mixture of $d$, $c$ and $b$ quark's wave functions and is determined by the quark mixing matrix which is also called Cabbibo-Kobayashi-Maskawa matrix \cite{ref_Pich}:\\
...
$B_\mu$, $W_\mu^1$, $W_\mu^2$, $W_\mu^3$ are four vector bosons that arise from the requirement of the Largangian to be invariant under local $SU(2)_L \times U(1)$ transformations.\\
...
In order to be consistent with QED, terms involving $A_\mu$ in the electroweak Largangian must be equal to the corresponding terms in QED Lagrangian~\cite{ref_Pich}:\\
...
Writing $\tilde{W}_\mu$ in Eq.~\ref{eq:L_gauge_kin} explicitely, we will get TGC and QGC coupling terms:\\ 
...
and charged QGC Lagrangians in the form of Eq.~\ref{eq:L_QGC_2}:\\
...
However, it is known from experiments that a $Z$ boson, a $W$ boson and fermions are massive particles and, therefore, our theory should accomodate their masses. To introduce masses into the electroweak Lagrangian, an $SU(2)_L$ doublet of complex scalar fields $\phi(x)$ is added to the Lagrangian:\\
