...
Two MC closure checks are performed to validate the procedure. For the first check, MC samples $W\gamma$, $W$+jets, $Z\gamma$, $Z$+jets, and $t\bar{t}$+jets run through the $W\gamma$ selection conditions and mixed together to mimic real data. Then fits on this sample of pseudodata are performed and the fit results are superimposed with MC predictions same as it is done for the real data. The results of the MC closure check are reported in App.~\ref{sec:MCclosureCheck} and the fit plots are shown in App.~\ref{sec:TemplateFitPlotsMCclosure}. 
