...
The fits are performed in an extended unbinned maximum likelihood way, separately in each $P_T^\gamma$ bin in fine $\eta^\gamma$ binning. The $\eta^\gamma$ binning for different $P_T^\gamma$ ranges is described in Tab.~\ref{tab:fine_eta_binning}.
...
In addition to the backgrounds discussed before, there is also the real-$\gamma$ background. The contributions to this background from $Z\gamma \rightarrow ll\gamma$  and $W\gamma \rightarrow \tau \nu \gamma$ are estimated based on MC predictions.
