...
\end{equation}
\noindent{where the sum runs over individual crystals in the~$5\times5$ matrix around the crystal that detects the largest $P_T$, and $w_i$ is the weight that has a logarithmic dependence on energy released by the photon.}
...
The distributions of both $I_{ch}^{\gamma}$ and $\sigma_{i\eta i\eta}^{\gamma}$ depend on $\eta$. Therefore, all templates are prepared separately for barrel and endcap photons.

To prepare fake-$\gamma$ templates, we need a sample that consists of jets reconstructed and identified as photons. The $Z\gamma\rightarrow\mu\mu\gamma$-selected dataset consists of $Z\gamma$ and (DY+jets) events, where jets from the DY+jets are reconstructed and identified as photons, same as jets from $W$+jets, DY+jets and $t\bar{t}$+jets in a $W\gamma$-selected sample. In order to increase a fraction of jets, on top of the nominal $Z\gamma$ selection conditions decribed in Ch.~\ref{sec:AN_Selection_EventLevel}, we apply ISR requirements. The ISR requirements include the lepton-photon separation $\Delta{R_{min}}(\mu,\gamma)>1.0$, and the invariant mass of the three final state particles $M_{ll\gamma}>101$~GeV. The remaining non-negligible real-$\gamma$ contribution is subtracted using $Z\gamma$ MC predictions.
...
The fits are performed in an extended unbinned maximum likelihood way, separately in each $P_T^\gamma$ bin in fine $\eta^\gamma$ binning. We use fine $\eta^{\gamma}$ binning because an electron track ability to be reconstructed and, therefore, the the amount of the $e\rightarrow\gamma$ background, depends strongly on $\eta$. The $\eta^\gamma$ binning for different $P_T^\gamma$ ranges is described in Tab.~\ref{tab:fine_eta_binning}. The fit outputs in fine $\eta$ bins are summed up to form EB and EE yields. 
...
The $M_{e\gamma}$ distribution has two distinct types of events. The first is the events from DY+jets$\rightarrowee$+jets with one of the electrons misidentified as photons. The distribution of $M_{e\gamma}$ of these events has a $Z$ peak and a non-resonant component rising to low masses. The second type of events are all the other sources that do not have a $Z$ peak. The general view of the fit function is:
...
The fit plots as well as the explanaition of parameters on the plots are provided in App.~\ref{sec:EtogammaFitPlots} and the tables with the parameter values determined from the fits in different $P_T^{\gamma}$ ranges separately for EB and EE final state photons are provided in App.~\ref{sec:etogTables}.
...
The results produced with two methods of jets$\rightarrow\gamma$ background estimation differ significantly, and both show significant disagreement with the MC prediction. To understand the disagreements and validate the procedure, we perform two MC closure checks (Ch.~\ref{sec:AN_BackSubtr_Closure}). 
