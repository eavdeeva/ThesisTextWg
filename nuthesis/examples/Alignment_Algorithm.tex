...
\noindent{where $\mathbf{p}$ are parameters describing the tracker geometry, $\mathbf{q}_j$ are parameters of the $j^{th}$ track, $m_{ij}-f_{ij}$ are residuals (distances between the measured hit and a position predicted by the track fit), $\sigma_{ij}$ is the Gaussian error of the measurement.}
