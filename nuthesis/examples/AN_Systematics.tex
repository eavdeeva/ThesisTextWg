...
The systematic uncertainty due to the difference between fit results of $I_{ch}$ and $\sigma_{i\eta i\eta}$ templates is computed as (1/2)($|N_{Ich}-N_{\sigma i\eta i\eta}|$). The mean value is taken as an average between $N_{Ich}}$ and $N_{\sigma{i\eta i\eta}}$.
...
\begin{itemize}
% MAYBE, DESCRIPTION IS TOO TECHNICAL
  \item randomize templates N times as mean = random.Gauss(mean,sigma), set error = sigma for each bin in a template (mean is a bin content of a given bin, sigma is determined by a statistical uncertainty of FSR(ISR) data sample for real-$\gamma$(fake-$\gamma$) templates and statistical uncertainty ofthe  leakage sample: DY+jets-MC($Z\gamma$-MC);
  ...
  \item propagate through unfolding, $A \times \epsilon$ correction, divide over luminosity and bin width;
  \item choose $N=100$ for fake-$\gamma$ templates and $N=20$ for real-$\gamma$ templates.
\end{itemize}
\noindent{The statistical error of the fake-$\gamma$ template is larger than one of the real-$\gamma$ template.}
