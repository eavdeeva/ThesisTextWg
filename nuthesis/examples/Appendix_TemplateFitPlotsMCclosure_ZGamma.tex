...
This appendix contains fit results of jets$\rightarrow \gamma$ data-driven estimation performed on pseudodata prepared as MC samples DY+jets and $Z\gamma$ with $Z\gamma$ selection conditions applied. The samples are appropriately weighted and mixed together. 

Results of fits of $I_{ch}^{\gamma}$ and $\sigma_{i\eta i\eta}$ templates are shown in Fig.~\ref{fig:templateFits_MCclosure_CHISO_MUON_ZGamma_0}-\ref{fig:templateFits_MCclosure_CHISO_ELECTRON_ZGamma_3} and Fig.~\ref{fig:templateFits_MCclosure_SIHIH_MUON_ZGamma_0}-\ref{fig:templateFits_MCclosure_SIHIH_ELECTRON_ZGamma_3} respectively. On all the plots, black is pseudodata, green is real-$\gamma$ template, blue is fake-$\gamma$ template, and red is the fit function. Cyan is a histogram of DY+jets MC sample in a given channel. The fits on pseudodata is performed for MC closure check. 
