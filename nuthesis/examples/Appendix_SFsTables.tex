...
This appendix summarize efficiency SF that were applied on MC events to make selection efficiency in MC match the selection efficiency in data. Tables~\ref{tab:SFs_MuonIso}-{tab:SFs_MuonIso} contain muon~ID ($\rho^{\mu}_{ID}$) and muon isolation SF ($\rho^{\mu}_{iso}$). A full SF on a muon object is a multiplication of~ID and isolation SF. Electron~ID SF are listed in Tab.~\ref{tab:SFs_ElectronID} ($\rho^{e}_{ID}$). Photon~ID ($\rho^{\gamma}_{ID}$) and PSV ($\rho^{\gamma}_{PSV}$) SF are summarized in Tab.~\ref{tab:SFs_PhotonID} and Tab.~\ref{tab:SFs_PhotonPixelSeedVeto}.
