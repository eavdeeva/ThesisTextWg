...
The cross section is computed as described in the beginning of Ch.~\ref{sec:AN_WgMeas}. For comparison purposes, in addition to the measured cross section, we also estimate the cross section beased on the signal MC sample.


The cross section of the whole simulated sample was computed with MCFM~\cite{ref_MCFM} in NLO and is for the dedicated signal MC sample $\sigma(sample)$=553.92 pb. The MC sample was generated with MadGraph~\cite{ref_MadGraph}, and the cross section in our selected phase space was computed as 

\begin{center}
$\sigma(phase space) = \sigma(sample) \cdot N_{phase space} / N_{sample}$, 
\end{center} 

\noindent{where  $N_{phase space}$ and $N_{sample}$ are numbers of events falling into selected phase space and generated in the whole MC sample respectively. For the differential cross section, $N_{phase space}$ is number of events falling into specific $P_T^{\gamma}$ bin and to compute }
...
For additional cross check, we estimated cross section of $Z\gamma$ and compared the result with the published CMS result for $Z\gamma$ at~$8$~TeV. The summary of this $Z\gamma$ check is available in App.~\ref{sec:ZgCheck}.
