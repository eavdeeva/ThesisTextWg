...
\end{equation}
\noindent{where $N_{j}$ and $N_{i}$ are numbers of events in a given $P_T^{\gamma(reco)}$ and $P_T^{\gamma(true)}$ bins, respectively, $R_{ji}$ is the ``response matrix'' where each element is the probability of an event with true $P_T^{\gamma}$ in the bin ``$i$'' to be reconstructed with  $P_T^{\gamma}$ in the bin ``$j$''.}
