...
\end{equation}
\noindent{where $N_{ireco}$ and $N_{itrue}$ are numbers of events in a given $P_T^{\gamma(reco)}$ and $P_T^{\gamma(true)}$ bins, respectively, $R_{ireco,itrue}$ is the ``response matrix'' where each element is the probability of an event with true $P_T^{\gamma}$ in the bin ``$itrue$'' to be reconstructed with  $P_T^{\gamma}$ in the bin ``$ireco$''.}
...
%\begin{equation}
%  N^{true}_i = \left( R_{ji} \right) ^{-1} N^{reco}_j.
%\end{equation}
